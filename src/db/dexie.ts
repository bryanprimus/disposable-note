...
/**
 * Detects the user's browser environment and provides the exact physical macOS path
 * where IndexedDB LevelDB/SQLite files are kept on disk.
 */
export function detectBrowserStorageLocation(): {
  browser: string;
  diskPath: string;
  idbDatabase: string;
} {
  const ua = typeof navigator !== 'undefined' ? navigator.userAgent : '';
  let browser = 'Chromium / Chrome';
  let diskPath =
    '~/Library/Application Support/Google/Chrome/Default/IndexedDB/http..._0.indexeddb.leveldb/';

  if (ua.includes('Edg/')) {
    browser = 'Microsoft Edge';
    diskPath =
      '~/Library/Application Support/Microsoft Edge/Default/IndexedDB/http..._0.indexeddb.leveldb/';
  } else if (ua.includes('Brave')) {
    browser = 'Brave Browser';
    diskPath =
      '~/Library/Application Support/BraveSoftware/Brave-Browser/Default/IndexedDB/http..._0.indexeddb.leveldb/';
  } else if (ua.includes('Firefox')) {
    browser = 'Mozilla Firefox';
    diskPath =
      '~/Library/Application Support/Firefox/Profiles/<profile>/storage/default/http.../idb/';
  } else if (ua.includes('Safari') && !ua.includes('Chrome')) {
    browser = 'Apple Safari';
    diskPath =
      '~/Library/Containers/com.apple.Safari/Data/Library/WebKit/WebsiteData/Default/ (SQLite3)';
  }

  return {
    browser,
    diskPath,
    idbDatabase: 'DisposableNoteDB (IndexedDB)',
  };
}
